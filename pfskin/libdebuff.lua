...
-- A pfSkin library that detects and saves all ongoing debuffs of players, NPCs and enemies.
-- 
...
if pfSkin.client > 11200 then return end

-- return instantly when another libdebuff is already active
if pfSkin.api.libdebuff then return end
...
pfSkin.libdebuff_own = pfSkin.libdebuff_own or {}
local ownDebuffs = pfSkin.libdebuff_own

-- allAuraCasts: [targetGUID][spellName][casterGuid] = {startTime, duration, rank}
-- Timer data for ALL debuffs (multi-caster support)
pfSkin.libdebuff_all_auras = pfSkin.libdebuff_all_auras or {}
local allAuraCasts = pfSkin.libdebuff_all_auras
...
pfSkin.libdebuff_slot_ownership = pfSkin.libdebuff_slot_ownership or {}
local slotOwnership = pfSkin.libdebuff_slot_ownership
...
pfSkin.libdebuff_display_to_aura = pfSkin.libdebuff_display_to_aura or {}
local displayToAura = pfSkin.libdebuff_display_to_aura

-- pendingCasts: [targetGUID][spellName] = {casterGuid, rank, time}
-- Temporary storage from SPELL_GO to correlate with DEBUFF_ADDED
pfSkin.libdebuff_pending = pfSkin.libdebuff_pending or {}
local pendingCasts = pfSkin.libdebuff_pending

-- Spell Icon Cache: [spellId] = texture
pfSkin.libdebuff_icon_cache = pfSkin.libdebuff_icon_cache or {}
local iconCache = pfSkin.libdebuff_icon_cache

-- Cast Tracking: [casterGuid] = {spellID, spellName, icon, startTime, duration, endTime}
-- Shared with nameplates for cast-bar display
pfSkin.libdebuff_casts = pfSkin.libdebuff_casts or {}
pfSkin.libdebuff_item_icons = pfSkin.libdebuff_item_icons or {}  -- [casterGuid] = icon (persists across SPELL_GO)

-- Cleveroids API: [targetGUID][spellID] = {start, duration, caster, stacks}
pfSkin.libdebuff_objects_guid = pfSkin.libdebuff_objects_guid or {}
local objectsByGuid = pfSkin.libdebuff_objects_guid

-- LEGACY: Keep these for backwards compatibility (external modules might check them)
pfSkin.libdebuff_own_slots = pfSkin.libdebuff_own_slots or {}
pfSkin.libdebuff_all_slots = pfSkin.libdebuff_all_slots or {}

-- Deduplication: Track recent AURA_CAST events to ignore duplicates
-- [targetGuid][spellName][casterGuid] = timestamp
pfSkin.libdebuff_recent_casts = pfSkin.libdebuff_recent_casts or {}
local recentCasts = pfSkin.libdebuff_recent_casts

-- Callbacks fired after SPELL_GO_SELF is processed: fn(spellId, arg1, arg2, arg3, arg4, arg5, arg6, arg7)
pfSkin.libdebuff_spell_go_hooks = pfSkin.libdebuff_spell_go_hooks or {}

-- Callbacks fired after SPELL_GO_OTHER is processed: fn(spellId, casterGuid, targetGuid)
pfSkin.libdebuff_spell_go_other_hooks = pfSkin.libdebuff_spell_go_other_hooks or {}

-- Callbacks fired after SPELL_START_SELF is processed: fn(spellId, casterGuid, targetGuid, castTime)
pfSkin.libdebuff_spell_start_self_hooks = pfSkin.libdebuff_spell_start_self_hooks or {}

-- Callbacks fired after SPELL_START_OTHER is processed: fn(spellId, casterGuid, targetGuid, castTime)
pfSkin.libdebuff_spell_start_other_hooks = pfSkin.libdebuff_spell_start_other_hooks or {}

-- Callbacks fired after SPELL_FAILED_OTHER is processed: fn(casterGuid, spellId)
pfSkin.libdebuff_spell_failed_other_hooks = pfSkin.libdebuff_spell_failed_other_hooks or {}

-- Callbacks fired after AURA_CAST_ON_SELF is processed: fn(spellId, casterGuid, targetGuid)
pfSkin.libdebuff_aura_cast_on_self_hooks = pfSkin.libdebuff_aura_cast_on_self_hooks or {}

-- Callbacks fired after AURA_CAST_ON_OTHER is processed: fn(spellId, casterGuid, targetGuid)
pfSkin.libdebuff_aura_cast_on_other_hooks = pfSkin.libdebuff_aura_cast_on_other_hooks or {}

-- Callbacks fired after DEBUFF_ADDED_OTHER is processed: fn(guid, luaSlot, spellId, stackCount)
pfSkin.libdebuff_debuff_added_other_hooks = pfSkin.libdebuff_debuff_added_other_hooks or {}

-- Callbacks fired after DEBUFF_REMOVED_OTHER is processed: fn(guid, luaSlot, spellId, stackCount)
pfSkin.libdebuff_debuff_removed_other_hooks = pfSkin.libdebuff_debuff_removed_other_hooks or {}

-- Callbacks fired after UNIT_HEALTH is processed: fn(unitToken)
pfSkin.libdebuff_unit_health_hooks = pfSkin.libdebuff_unit_health_hooks or {}

-- Callbacks fired after PLAYER_TARGET_CHANGED is processed: fn()
pfSkin.libdebuff_player_target_changed_hooks = pfSkin.libdebuff_player_target_changed_hooks or {}

-- Callbacks fired after UNIT_DIED is processed: fn(guid)
pfSkin.libdebuff_unit_died_hooks = pfSkin.libdebuff_unit_died_hooks or {}

-- Callbacks fired after SPELL_CAST_EVENT is processed: fn(success, spellId, castType, targetGuid)
pfSkin.libdebuff_spell_cast_hooks = pfSkin.libdebuff_spell_cast_hooks or {}
...
pfSkin.libdebuff_downrank_blocked_hooks = pfSkin.libdebuff_downrank_blocked_hooks or {}
...
pfSkin.libpredict_pending_cast = pfSkin.libpredict_pending_cast or {}
...
StaticPopupDialogs["LIBDEBUFF_NAMPOWER_UPDATE"] = {
  ...
  preferredIndex = 3,
  OnAccept = function()
    pfSkin.chat.urlcopy.CopyText("https://github.com/me0wg4ming/pfSkin/wiki/How-to-install-nampower")
  end,
}

StaticPopupDialogs["LIBDEBUFF_NAMPOWER_MISSING"] = {
  text = "|cffff0000!!!WARNING!!!|r\n\nNampower Not Found!\n\nNampower 3.0.0+ is required for pfSkin to function correctly.\n\nPlease install Nampower!",
  button1 = "Show Download",
  button2 = "Dismiss",
  timeout = 0,
  whileDead = 1,
...
-- Debug Stats
pfSkin.libdebuff_debugstats = pfSkin.libdebuff_debugstats or {
  enabled = false,
  trackAllUnits = false,
  aura_cast = 0,
  debuff_added = 0,
...
local debugStats = pfSkin.libdebuff_debugstats
...
pfSkin.libdebuff_lastranks = pfSkin.libdebuff_lastranks or {}
local lastCastRanks = pfSkin.libdebuff_lastranks

-- Speichert Spells die gefailed sind
pfSkin.libdebuff_lastfailed = pfSkin.libdebuff_lastfailed or {}
local lastFailedSpells = pfSkin.libdebuff_lastfailed
...
pfSkin.libdebuff_GetSpellIcon = function(spellId)
...
local function CleanupOutOfRangeUnits()
  ...
  if not pfSkin.libdebuff_last_pending_cleanup then pfSkin.libdebuff_last_pending_cleanup = 0 end
  if now - pfSkin.libdebuff_last_pending_cleanup >= 1 then
    pfSkin.libdebuff_last_pending_cleanup = now
...
function libdebuff:UpdateUnits()
  if not pfSkin.uf or not pfSkin.uf.target then return end
  pfSkin.uf:RefreshUnit(pfSkin.uf.target, "aura")
end
...
if hasNampower then
  ...
  carnageCheckFrame:SetScript("OnUpdate", function()
    ...
    if cp > 0 then
      ...
      if pfSkin.nameplates and pfSkin.nameplates.OnAuraUpdate then
        pfSkin.nameplates:OnAuraUpdate(guid)
      end
    end
  ...
  frame:SetScript("OnEvent", function()
    ...
    elseif event == "SPELLCAST_CHANNEL_STOP" then
      ...
      local myGuid = GetPlayerGUID()
      local castData = myGuid and pfSkin.libdebuff_casts[myGuid]
      if castData and castData.event == "CHANNEL" and castData.spellName then
        ...
        pfSkin.libdebuff_casts[myGuid] = nil
      end
    ...
    elseif event == "UNIT_HEALTH" then
      ...
      if pfSkin.libdebuff_unit_health_hooks then
        for _, fn in pairs(pfSkin.libdebuff_unit_health_hooks) do
          fn(arg1)
        end
      end
    ...
    elseif event == "SPELL_START_SELF" or event == "SPELL_START_OTHER" then
      ...
      if itemId and itemId > 0 and GetItemStatsField and GetItemIconTexture then
        ...
        pfSkin.libdebuff_item_icons[casterGuid] = {
          icon = icon,
      ...
      if event == "SPELL_START_SELF" and pfSkin.libdebuff_spell_start_self_hooks then
        for _, fn in pairs(pfSkin.libdebuff_spell_start_self_hooks) do
          fn(spellId, casterGuid, arg4, castTime)
        end
      elseif event == "SPELL_START_OTHER" and pfSkin.libdebuff_spell_start_other_hooks then
        for _, fn in pairs(pfSkin.libdebuff_spell_start_other_hooks) do
          fn(spellId, casterGuid, arg4, castTime)
        end
      end
    ...
    elseif event == "SPELL_GO_SELF" or event == "SPELL_GO_OTHER" then
      ...
      if casterGuid and pfSkin.libdebuff_casts[casterGuid] then
        if pfSkin.libdebuff_casts[casterGuid].spellID == spellId and pfSkin.libdebuff_casts[casterGuid].event ~= "CHANNEL" then
          pfSkin.libdebuff_casts[casterGuid] = nil
        end
      end
      
      -- Fire registered SPELL_GO_SELF hooks BEFORE miss guard
      -- (Swingtimer needs to see ALL casts, even misses, for swing reset)
      if event == "SPELL_GO_SELF" and pfSkin.libdebuff_spell_go_hooks then
        for _, fn in pairs(pfSkin.libdebuff_spell_go_hooks) do
        ...
        if castRank > 0 then
          ...
          if existingOwn and existingOwn.rank and existingOwn.rank > castRank then
            ...
            if existingTimeleft > 0 then
              ...
              if pfSkin.libdebuff_downrank_blocked_hooks then
                for _, fn in pairs(pfSkin.libdebuff_downrank_blocked_hooks) do
      ...
      if event == "SPELL_GO_SELF" and targetGuid and not isNullTarget then
        local selfdebuffMode = pfSkin_config and pfSkin_config.buffbar and
          pfSkin_config.buffbar.tdebuff and pfSkin_config.buffbar.tdebuff.selfdebuff == "1"
      ...
      if event == "SPELL_GO_OTHER" and pfSkin.libdebuff_spell_go_other_hooks then
        for _, fn in pairs(pfSkin.libdebuff_spell_go_other_hooks) do
          fn(spellId, casterGuid, targetGuid)
        end
      end

    elseif event == "UNIT_DIED" then
      if pfSkin.libdebuff_unit_died_hooks then
        for _, fn in pairs(pfSkin.libdebuff_unit_died_hooks) do
          fn(arg1)
        end
      end
    ...
    elseif event == "SPELL_FAILED_OTHER" then
      ...
      if casterGuid and pfSkin.libdebuff_casts[casterGuid] then
        ...
        if pfSkin.libdebuff_casts[casterGuid].spellID == spellId then
          pfSkin.libdebuff_casts[casterGuid] = nil
        end
      end
      if pfSkin.libdebuff_spell_failed_other_hooks then
        for _, fn in pairs(pfSkin.libdebuff_spell_failed_other_hooks) do
          fn(casterGuid, arg2)
        end
      end
    ...
    elseif event == "SPELL_CAST_EVENT" then
      ...
      if spellName and targetGuid and targetGuid ~= "" and targetGuid ~= "0x0000000000000000" then
        pfSkin.libpredict_pending_cast.spellId = spellId
        pfSkin.libpredict_pending_cast.spellName = spellName
        pfSkin.libpredict_pending_cast.targetGuid = targetGuid
        pfSkin.libpredict_pending_cast.time = GetTime()
      else
        -- No explicit target - clear pending so libpredict falls back to spell_queue
        pfSkin.libpredict_pending_cast.spellId = nil
        pfSkin.libpredict_pending_cast.spellName = nil
        pfSkin.libpredict_pending_cast.targetGuid = nil
        pfSkin.libpredict_pending_cast.time = nil
      end
      ...
      if pfSkin.libdebuff_spell_cast_hooks then
        for _, fn in pairs(pfSkin.libdebuff_spell_cast_hooks) do
    ...
    elseif event == "AURA_CAST_ON_SELF" or event == "AURA_CAST_ON_OTHER" then
      ...
      -- Notify nameplates
        if pfSkin.nameplates and pfSkin.nameplates.OnAuraUpdate then
          pfSkin.nameplates:OnAuraUpdate(targetGuid)
        end
      ...
      if event == "AURA_CAST_ON_SELF" and pfSkin.libdebuff_aura_cast_on_self_hooks then
        for _, fn in pairs(pfSkin.libdebuff_aura_cast_on_self_hooks) do
          fn(spellId, casterGuid, targetGuid)
        end
      elseif event == "AURA_CAST_ON_OTHER" and pfSkin.libdebuff_aura_cast_on_other_hooks then
        for _, fn in pairs(pfSkin.libdebuff_aura_cast_on_other_hooks) do
          fn(spellId, casterGuid, targetGuid)
        end
      end
    ...
    elseif event == "DEBUFF_ADDED_OTHER" then
      ...
      if pfSkin.libdebuff_debuff_added_other_hooks then
        for _, fn in pairs(pfSkin.libdebuff_debuff_added_other_hooks) do
    ...
    elseif event == "DEBUFF_REMOVED_OTHER" then
      ...
      if pfSkin.libdebuff_debuff_removed_other_hooks then
        for _, fn in pairs(pfSkin.libdebuff_debuff_removed_other_hooks) do
          fn(arg1, arg2, arg3, arg4)
        end
      end
    ...
    elseif event == "PLAYER_TARGET_CHANGED" then
      ...
      if pfSkin.libdebuff_player_target_changed_hooks then
        for _, fn in pairs(pfSkin.libdebuff_player_target_changed_hooks) do
          fn()
        end
      end
    end
...
-- add libdebuff to pfSkin API
pfSkin.api.libdebuff = libdebuff
